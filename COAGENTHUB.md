...
- **默认先讨论,再下发**:收到实现类需求后,默认先围绕**需求本身**进行澄清,例如:
  - 目标是否理解正确:1. 是 / 2. 需要修正;
  - 范围是否包含某模块:1. 包含 / 2. 不包含;
  - 验收标准:1. 只看测试通过 / 2. 测试通过+人工确认。
- 等用户确认后,再调用 `coagenthub_dispatch_task`。
- **可以直接下发的例外**:
  - 用户明确说"直接派 / 不用讨论 / 下发吧 / 派"等——此时**不需要澄清,直接下发**;
  - 用户已经确认过任务书;
  - 纯查询类操作(查群 / 查任务 / 查消息)不在此列。
- **避免过度讨论**:
  - 用户已经给出明确任务书或确认过方案时,不要重复问相同问题;
  - 澄清只围绕需求本身不清晰的点,不要每次从零开始;
  - 执行器选择等由 agent 按现有规则自动决定,不要反复问用户。

### 澄清选项的编号呈现

- 澄清选项**只用编号给出**,用户回复编号即可,例如:

```text
1. 目标是否理解正确：1. 是 / 2. 需要修正
2. 范围是否包含某模块：1. 包含 / 2. 不包含
3. 验收标准：1. 只看测试通过 / 2. 测试通过+人工确认
```

- 选项只用于需求澄清,不提供执行器 / 范围选择器;
- 用户回复编号后,**直接按选择执行,不要重新解释所有选项**;
- 选项不超过 3 个时默认编号 1/2/3;超过 3 个再考虑分组;
- 用户已经明确"直接派"时,不需要再给选项。
...
`coagenthub_dispatch_task` 用结构化参数 `goal / scope / acceptance / tests / report /
priority / dependencies` 渲染 Markdown 任务书(只传 `body` 时原样发送),并支持
`commitMode` / `specRef` / `specHash` / `planOnly`。完整参数与字段见仓库
`README.md` 的「工具清单」一节。

插件**架构分层 / 文件职责 / 后台事件链路 / 配置细节**统一以 `README.md` 为准
(单一事实源),不在此处重复,避免两处描述漂移过时。
